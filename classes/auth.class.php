...
class auth extends conexion{
        ...
        if(!isset($datos['usuario']) || !isset($datos['password'])){ //campos Json
          ...
          return $_respuestas->error_400();
           ...
           if(!password_verify($password,$datos[0]['Password'])){
            ...
            if($datos[0]['Estado']!= "Activo"){return $_respuestas->error_200("El usuario esta inactivo");} 
    ...
    private function obtenerDatosUsuario($userName){
        $query = "SELECT UserID,Password,Estado,Autor,Clinica FROM users WHERE UserID = '$userName'";
